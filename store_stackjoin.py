...
def store_stackjoin(json_response):
    ...
    for index, tweet in enumerate(stackjoin_tweets):
        ...
        stackjoin_tweets_table_data += (f"<tr><td>{index+1}</td><td><a href=\"https://www.twitter.com/pleblira/status/{tweet['tweet_id']}\">{tweet['tweet_id']}</a></td><td><a href=\"https://twitter.com/{tweet['author_handle']}\" target=\"_blank\">{tweet['author_handle']}</a></td><td>{tweet['author_id']}</td><td>{tweet['tweet_message']}</td><td>{str(tweet['image_url_dict'])}</td><td>{str(tweet['img_src_dict']).translate({39: None,91: None, 93: None, 44: None})}")
